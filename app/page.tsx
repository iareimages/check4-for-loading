...
function MusicPlayerContent() {
  ...
  useEffect(() => {
    console.log('🔍 App state:', { 
      ...
      personalizedCount: personalizedSongs.length 
    });
  }, [user, loading, songs.length, personalizedSongs.length]);
  ...
  const [volume, setVolume] = useState(1); // default 75%
  ...
  useEffect(() => {
    // Only reset state when user explicitly logs out, not on session recovery
    if (!user && !loading) {
      setCurrentSong(null)
      setIsPlaying(false)
...
useEffect(() => {
  ...
  fetchImages();
}, [displayCount, songs, trendingSongs, playlists, likedSongs, recentlyPlayedSongs]);
...
const handleNext = async () => {
  ...
  const renderContent = () => {
    ...
    if (currentPage === 'liked') {
      return <LikedSongsPage songs={likedSongs} onBack={() => setCurrentPage('main')} onSongPlay={handleSongPlay} onAddToQueue={handleAddToQueue} imageUrls={imageUrls} clearQueue={clearQueue}/>;
    }
    ...
    switch (activeTab) {
      ...
      default:
        return <HomePage
              ...
              imageUrls={imageUrls}
              onLoadMore={loadMoreSongs}
              hasMoreSongs={displayCount < personalizedSongs.length}
              trendingSongs={trendingSongs}
